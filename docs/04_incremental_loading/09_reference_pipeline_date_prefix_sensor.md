...
> 2. **Dynamic DTS Scoping:** The BigQuery DTS transfer configuration is parameterized to target only the active execution date's folder prefix using runtime parameters (`export_{run_time|"%Y%m%d"}/`) [^1], completely eliminating the risk of re-ingesting past partitions.
> 3. **Zero Deletion:** Zero files are deleted from GCS. The data lake remains immutable, auditable, and replayable.
> 4. **Race-Condition Elimination:** A dedicated `BigQueryDataTransferServiceTransferRunSensor` polls DTS until terminal completion before downstream tasks fire [^2].
> 5. **Idempotent MERGE:** An optimized BigQuery `MERGE` statement with partition pruning merges changes into the native serving layer [^3][^4].
...
### 2.1. Staging Table (BigLake Managed Iceberg) [^5]
```sql
...
Partitioned by `created_at` and clustered by `id_transaksi` for optimal point-lookups [^6][^7]:
```sql
...
7. **Cloud Storage URI:**  
   ...
   > [!TIP]
   > The `{run_time+7h}` macro dynamically offsets UTC execution time to GMT+7 (Asia/Jakarta), aligning the DTS pattern matching with Airflow's localized date directory [^1].
...
### A. Eliminating Race Conditions via DTS Sensor [^2]
...
### B. Strict PyArrow Schema Typing [^8]
...
### C. Partition-Pruned Idempotent MERGE [^3][^4]
