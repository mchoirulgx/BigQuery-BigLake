...
The architecture employs a two-tier approach to balance storage costs, historical auditability, and query performance <a href="https://cloud.google.com/architecture/datashare-lakehouse" target="_blank">[1]</a>:
...
Data types are mapped from MySQL relational columns into BigQuery standard SQL data types <a href="https://cloud.google.com/bigquery/docs/reference/standard-sql/data-types" target="_blank">[2]</a>:
...
Create the Iceberg managed staging table using BigLake DDL <a href="https://cloud.google.com/bigquery/docs/biglake-iceberg-tables-in-bigquery#create-tables" target="_blank">[3]</a>:
```sql
...
> [!NOTE]
> **Hidden Partitioning in Iceberg <a href="https://iceberg.apache.org/docs/latest/partitioning/" target="_blank">[4]</a>**: Unlike Hive external tables, you will not see folder paths like `created_at=2026-08-20/` in GCS. Iceberg maintains partition boundaries inside metadata manifests, preventing folder lock-in and allowing schema/partition evolution without table re-writes.
...
Standard BigQuery Load APIs (`bq load` or `to_gbq`) do not currently support direct streaming/loading into BigLake Managed Iceberg tables <a href="https://cloud.google.com/bigquery/docs/biglake-iceberg-tables-in-bigquery#create-tables" target="_blank">[3]</a> <a href="https://cloud.google.com/bigquery/docs/biglake-iceberg-tables-in-bigquery#limitations" target="_blank">[5]</a>. Attempting to use the load job API returns a `403 Delegate Permission` or unsupported destination error.

As an educational prototype for small demo datasets, this DAG demonstrates loading by serializing DataFrame records into JSON and executing a parameterized BigQuery `INSERT` query using JSON unnesting functions <a href="https://cloud.google.com/bigquery/docs/reference/standard-sql/json_functions" target="_blank">[7]</a>:
...
> While the JSON UNNEST approach works effectively for demo workloads (< 5,000 rows), BigQuery has a query string and parameter size limit of 1 MB <a href="https://cloud.google.com/bigquery/quotas#query_jobs" target="_blank">[6]</a>. For production datasets, data should be written to GCS as Parquet and loaded using **BigQuery Data Transfer Service (DTS)**, which is covered in Stages 3 and 4.

---

## 5. Merging Staging to Native Serving

Once new updates land in `history_users`, an idempotent `MERGE` updates the native serving table, transforming soft-deletes into hard deletes <a href="https://cloud.google.com/bigquery/docs/reference/standard-sql/dml-syntax#merge_statement" target="_blank">[8]</a>:
...
1. <a href="https://cloud.google.com/architecture/datashare-lakehouse" target="_blank">Google Cloud Architecture Center - Datashare lakehouse architecture</a>
2. <a href="https://cloud.google.com/bigquery/docs/reference/standard-sql/data-types" target="_blank">Google Cloud - BigQuery standard SQL data types</a>
...
4. <a href="https://iceberg.apache.org/docs/latest/partitioning/" target="_blank">Apache Iceberg - Hidden Partitioning Specification</a>
5. <a href="https://cloud.google.com/bigquery/docs/biglake-iceberg-tables-in-bigquery#limitations" target="_blank">Google Cloud - BigLake Iceberg table limitations</a>
6. <a href="https://cloud.google.com/bigquery/quotas#query_jobs" target="_blank">Google Cloud - BigQuery Quotas & Limits (Query Jobs)</a>
7. <a href="https://cloud.google.com/bigquery/docs/reference/standard-sql/json_functions" target="_blank">Google Cloud - BigQuery JSON functions and operators</a>
8. <a href="https://cloud.google.com/bigquery/docs/reference/standard-sql/dml-syntax#merge_statement" target="_blank">Google Cloud - BigQuery MERGE DML syntax</a>
