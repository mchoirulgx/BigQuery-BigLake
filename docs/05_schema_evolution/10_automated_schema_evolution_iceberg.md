...
> **Section Overview:**  
> In real-world data pipelines, upstream database schemas change: new columns are added over time. While BigQuery Native tables support `schema_update_options = ['ALLOW_FIELD_ADDITION']`, BigQuery DTS and standard Load APIs **do not currently support automated schema evolution for BigLake Managed Apache Iceberg tables**. Attempting to load files with new columns causes DTS to abort with a schema mismatch error.  
> This section presents an automated hybrid workaround: an Airflow pre-load task inspects incoming Parquet metadata in GCS via PyArrow, applies `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` dynamically, and only then triggers DTS to ingest the data without errors.
...
1. **Task 1 (`check_and_evolve_schema`)**:
   - Inspects the incoming Parquet file metadata directly in GCS using `pyarrow.parquet.read_schema`.
   ...
   - Executes `ALTER TABLE <table_id> ADD COLUMN IF NOT EXISTS <col> <type>`.
2. **Task 2 (`trigger_dts_transfer`)**:
   - Triggers the on-demand BigQuery Data Transfer Service (DTS) transfer run. Because the table schema has already been pre-expanded to match the Parquet file, DTS ingests the records smoothly without rejection.
...
Initialize your managed Iceberg table in BigQuery with its initial schema:
